...
APP_PORT=$(grep -E '^PORT=' "${APP_DIR}/.env" 2>/dev/null | cut -d '=' -f2 | tr -d '"' | tr -d "'" | tr -d ' ' || echo 3005)
...
if [ "$HTTP_CODE" != "200" ]; then
  ...
  cd "$APP_DIR" && (pm2 reload ecosystem.config.cjs --update-env || pm2 start ecosystem.config.cjs)
else
